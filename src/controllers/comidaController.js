...
export const getAll = async (req, res) => {
    try {
        const comidas = await model.findAll(req.query);
...
export const create = async (req, res) => {
    try {
        ...
        if (!descricao) return res.status(400).json({ error: 'O ano (descricao) é obrigatório!' });
        if (!categoria) return res.status(400).json({ error: 'O preço (categoria) é obrigatório!' });
